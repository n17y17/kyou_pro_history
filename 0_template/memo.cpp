...
int main() {
    ...
    *(p + i) = p[i] = i[p] である（両方とも要素を表し，ポインタではない）同様にa[i]のaは配列aの先頭へのポインタだと解釈できる．
    ...
    累積和 / 累積max
        一つの配列が与えられ，複数のクエリ（a番目からb番目までの和を求めるもの）を実行するとき
        事前に0からbまでの和sの配列を作って置くといい
        i番目を除く-->両側からの累積を考える
    a[i]*1 + a[i+1]*2 + a[i+3]*3 + ...のiを変えていく
        初めの値を用意して，次の数を足して，累積輪を引く
    
    イテレーター間の距離
        イテレーターの引き算

    DP 動的計画法 Direct Programing
        部分問題を考える
        選択を繰り返して，ある数を最大にしたい
        i*jの配列dpを作る．i:番目，j:状態，dp:最大にしたい数
        変化を考えて漸化式を立てるのが重要
        もらうDPと配るDPがある
        i番目を計算するときに，i-1番目までを使う
        i番目の時に，i+1番目以降に足す（操作する）
        DPは表を作るといい
        最長共通部分列
            空文字から始めて，二つの文字列の文字ごとの表を作る．
            i番目とj番目の文字が同じとき

    配列の大きいほうからk番目までを特殊処理
        配列(multiset)をk番目までとk以降で分けて保存する
        multiset：同じ要素も含んでよい集合
        ある数を含んでいるかの判定は.countでは時間がかかるので.findがendではないことを使う！

    nで割った切り上げを求める
        (x + (n-1)) / n

    dequeのイテレータ
        pop_frontをした場合，最初の要素以外のイテレータは保持される
        ただし，最初の要素のイテレータは無効化されるので，範囲for文は使えない
        最初と最後以外をeraseした場合，すべてのイテレータは無効化され，削除した次の値へのイテレータが戻り値として返ってくる
    ...
    グラフ理論
        a-b-c-d
         ...
         \g-h
           \i
        のとき ↑:木, a:根, dfhi:葉, 直径:一番長く続いている辺の数
    
    Union Find
        グラフで，同じ木に属するかを判定する手法
        要素の連結，同じ木かどうかの判定ができる

    隣接行列
        グラフのつながりを二次元配列で表したもの
        無向グラフの場合は線対称になる
        0-1-2
         \3/
          0123
    ...
    構造体
        ...
        タグ名は慣習として先頭に_をつける．タグ名は型名ではないのでstruct タグ名のように並べて使う．
        typedef struct _タグ名 新しい型名_t でstructをつけなくともよくなる．
        （typedefで作る新しい型名には慣習として_tをつける）
        新しい型名をtypedefでつける場合にはタグ名を書かず，次の書き方でよい
        typedef struct {
            型名 変数名;
        } 新しい型名_t;
    ...
    マンハッタン距離 --> 45°回転
        ...
        |x| = max(x, -x) を使って変形
        zi = xi + yi,     wi = xi - yiとすると，
        マンハッタン距離：max(|z1-z2|, |w1-w2|)
        --> 最大値：max(|max(zi)-min(zi)|, |max(wi)-min(wi)|)
    */
    return 0;
}